...
try:
    ...
    from requests.exceptions import RequestException as _RequestException  # type: ignore[misc]
except ImportError:
    requests = None  # type: ignore[assignment]
    _RequestException = Exception  # type: ignore[misc,assignment]

pytestmark = pytest.mark.skipif(
    requests is None, reason="requests library not installed (optional dependency)"
)
...
class TestHTTPSnapshotInit:
    ...
    def test_custom_dcc_type(self) -> None:
        snap = HTTPSnapshot(
            base_url="http://host", dcc_type="unreal", timeout=60.0
        )
...
class TestHTTPCaptureUnreal:
    ...
    def test_unreal_capture_error_in_response(self, mock_session) -> None:
        ...
        mock_response.json.return_value = {
            "Error": "Object not found: invalid path"
        }
        mock_response.raise_for_status = MagicMock()
        mock_session.post.return_value = mock_response
...
class TestHTTPSnapshotConfigPassing:
    ...
    def test_config_width_height_in_params(self, mock_session) -> None:
        ...
        snap = HTTPSnapshot(
            base_url="http://host", dcc_type="unity", session=mock_session
        )
