...
Design notes:
- Uses only stdlib ``threading`` and ``socket`` — no heavy external dependencies
  required on the DCC side.
- ``websockets`` library is optional; if absent, a ``NotImplementedError`` is raised
  at connect time (graceful degradation).
...
class WebSocketTransport(BaseTransport):
    ...
    Example::
        ...
        ws.subscribe("selection_changed", lambda ev, data: print(data))
        ws.disconnect()
    ...
    def execute(
        ...
        try:
            ...
            if not event.wait(timeout=effective_timeout):
                ...
                raise TimeoutError(
                    f"Action '{action}' timed out after {effective_timeout}s"
                )
