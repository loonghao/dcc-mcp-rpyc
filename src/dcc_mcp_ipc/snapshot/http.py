...
class HTTPSnapshot(BaseSnapshot):
    ...
    def _capture_unreal(self, config: SnapshotConfig) -> bytes:
        ...
        """
        try:
            # Unreal RC API: call take_screenshot on EditorLevelLibrary
    ...
    def _capture_unity(self, config: SnapshotConfig) -> bytes:
        ...
        try:
            ...
            raise SnapshotError(
                f"Unexpected response from Unity screenshot endpoint: "
                f"content-type={ct}"
            )
    ...
    def _capture_generic(self, config: SnapshotConfig) -> bytes:
        ...
        try:
            ...
            raise SnapshotError(
                f"Generic HTTP snapshot returned non-image content: {ct}"
            )
            ...
            if isinstance(exc, SnapshotError):
                raise
            raise SnapshotError(
                f"Generic HTTP snapshot failed: {exc}", cause=exc
            ) from exc
